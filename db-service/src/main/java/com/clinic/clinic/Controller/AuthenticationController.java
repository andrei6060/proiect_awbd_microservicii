...
public class AuthenticationController {
    ...
    @ResponseStatus(HttpStatus.ACCEPTED)
    public ResponseEntity<?> registerUserApi(@Valid @RequestBody RegistrationRequestDto request) throws MessagingException {
        authenticationService.register(request);
        return ResponseEntity.accepted().build();
    }

    @GetMapping("/activateUser2")
    public void confirm2(@RequestParam String token) throws MessagingException {
        authenticationService.activateAccount(token);
    }
